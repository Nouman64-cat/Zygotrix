...
const FloatingDNABases: React.FC = () => {
    const bases = React.useMemo(() => ['A', 'T', 'G', 'C', 'A', 'T', 'G', 'C', 'A', 'G', 'C', 'T'], []);
    const [baseStyles, setBaseStyles] = useState<{ fontSize: string; left: string; top: string; duration: string }[]>([]);

    React.useEffect(() => {
        setBaseStyles(bases.map(() => ({
