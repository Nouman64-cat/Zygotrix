...
const PolygenicOverview: React.FC<PolygenicOverviewProps> = ({ score, loading, error }) => {
  ...
  return (
    <div className="rounded-3xl bg-[#1E3A8A] p-6 text-white shadow-xl shadow-[#1E3A8A]/40">
      <p className="text-xs uppercase tracking-[0.35em] text-[#A5B4FC]">Polygenic signal</p>
      <p className="mt-2 text-2xl font-semibold">
        Expected score {" "}
        {loading ? "..." : score !== null ? score.toFixed(2) : "N/A"}
      </p>
      {error ? (
        <p className="mt-4 text-sm text-[#FBBF24]">{error}</p>
      ) : (
        <p className="mt-4 text-sm text-slate-200">
          Weighted SNP dosages averaged from both parents, refreshed live from the backend service.
        </p>
      )}
