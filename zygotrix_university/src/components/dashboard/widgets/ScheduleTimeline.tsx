...
const badgeStyles: Record<"live" | "async" | "deadline", string> = {
  ...
  async: "bg-blue-500/15 text-blue-500 border-blue-500/30",
  deadline: "bg-rose-500/15 text-rose-500 border-rose-500/30",
};
  ...
  return (
    <div className="rounded-[1.75rem] border border-border bg-surface p-6 transition-colors">
      <div className="flex items-center justify-between">
        ...
        <span className="inline-flex items-center gap-2 rounded-full border border-border bg-background-subtle px-3 py-1 text-xs font-semibold text-muted transition-colors">
          <FiZap /> 3 planned
        </span>
      </div>
        ...
        {events.map((event) => {
          ...
          return (
            <li key={event.id} className="relative pl-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-[1.25rem] border border-border bg-background-subtle px-4 py-3 transition-colors">
