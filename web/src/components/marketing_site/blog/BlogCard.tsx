...
const BlogCard: React.FC<BlogCardProps> = ({ blog }) => {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-blue-200/80 hover:shadow-xl hover:shadow-blue-500/10">
      ...
      >
        <div className="aspect-[3/2] w-full overflow-hidden bg-slate-100 relative">
          ...
          <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1 text-xs font-medium text-slate-700 opacity-80 transition-opacity group-hover:opacity-100">
            <svg
      ...
      <div className="relative flex flex-1 flex-col gap-4 p-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            onClick={handleShare}
            className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 transition-all hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 hover:shadow-sm"
        ...
        <div className="space-y-3 flex-1">
          <h3 className="text-xl font-bold leading-tight text-slate-900 group-hover:text-blue-700 transition-colors duration-200">
            <Link to={`/blogs/${blog.slug}`} className="line-clamp-2">
              {blog.title}
            </Link>
          </h3>
          <p className="line-clamp-3 text-sm leading-relaxed text-slate-600 group-hover:text-slate-700 transition-colors">
            {blog.excerpt}
          </p>
          ...
          {blog.categories && blog.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {blog.categories.slice(0, 2).map((category) => (
                <span
                  key={category.slug}
                  className="inline-flex items-center rounded-md bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 border border-blue-200/50"
                >
                  {category.title}
                </span>
              ))}
        ...
        <div className="mt-auto pt-4 border-t border-slate-100">
          <div className="flex items-center justify-between">
            ...
            <Link
              to={`/blogs/${blog.slug}`}
              className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 text-blue-600 opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-blue-100 hover:scale-110"
              aria-label={`Read ${blog.title}`}
            >
              <svg
                className="w-4 h-4"
