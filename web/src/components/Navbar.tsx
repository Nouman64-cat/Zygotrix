...
const Navbar: React.FC = () => {
  ...
  const linkClasses = ({ isActive }: { isActive: boolean }) =>
    `rounded-full px-4 py-2 text-sm font-semibold transition ${
      isActive ? "bg-white text-[#1E3A8A] shadow" : "text-white/80 hover:text-white"
    }`;

  return (
    <header className="sticky top-0 z-50 bg-gradient-to-b from-slate-900/95 via-slate-900/80 to-transparent backdrop-blur-lg">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <Link to="/" className="group flex items-center gap-3">
          ...
          <div className="leading-tight">
            <p className="text-base font-semibold uppercase tracking-[0.35em] text-white">Zygotrix</p>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `block rounded-2xl px-4 py-3 text-base font-semibold transition ${
                    isActive ? "bg-white text-[#1E3A8A]" : "text-white/80 hover:bg-white/10 hover:text-white"
                  }`
                }
              >
                {item.label}
