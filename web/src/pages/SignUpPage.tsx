      ...
      {steps.map((step, i) => (
        <React.Fragment key={step.key}>
          ...
          <div className="flex flex-col items-center">
            ...
            <span
              className={`mt-2 text-xs font-medium transition-colors duration-300 ${i <= currentIdx ? "text-slate-900" : "text-slate-400"
                }`}
            >
              {step.label}
            </span>
          ...
          {i < steps.length - 1 && (
            <div
              className={`w-16 sm:w-20 h-0.5 mx-2 transition-colors duration-300 ${i < currentIdx ? "bg-slate-900" : "bg-slate-100"
                }`}
...
const SignUpPage: React.FC = () => {
  useDocumentTitle("Sign Up | Zygotrix");
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.detail || err.message || "Failed to sign up. Please try again.";
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.detail || err.message || "Invalid code. Please try again.";
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.detail || err.message || "Failed to resend code. Please try again.";
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
