...
def send_signup_otp_email(
    ...
    if not settings.aws_ses_username or not settings.aws_ses_password:
        raise HTTPException(status_code=503, detail="Email service is not configured. Please set AWS_SES_USERNAME and AWS_SES_PASSWORD.")

    # Note: Removed development mode bypass to enable email sending in all environments
    ...
    try:
        ...
        logger.error(f"Unexpected error while sending OTP email: {exc}", exc_info=True)
        raise HTTPException(
            status_code=503, detail=f"Failed to send OTP email: {str(exc)}") from exc
