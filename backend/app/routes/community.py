...
def list_questions(
    ...
    sort_by: str = Query(
        "newest", pattern="^(newest|oldest|most_voted|most_viewed|most_answered)$"
    ),
...
def get_question(
    ...
    return QuestionDetailResponse(
        **question, answers=answers_response, comments=comments_response
    )
...
def update_question(
    ...
    if not success:
        raise HTTPException(
            status_code=403, detail="You can only update your own questions"
        )
...
def delete_question(
    ...
    if not success:
        raise HTTPException(
            status_code=403, detail="You can only delete your own questions"
        )
...
@router.post(
    "/questions/{question_id}/answers", response_model=AnswerResponse, status_code=201
)
def create_answer(
    question_id: str,
...
def update_answer(
    ...
    if not success:
        raise HTTPException(
            status_code=403, detail="You can only update your own answers"
        )
...
def delete_answer(
    ...
    if not success:
        raise HTTPException(
            status_code=403, detail="You can only delete your own answers"
        )
...
def create_comment(
    ...
    try:
        ...
        comment = db[settings.mongodb_comments_collection].find_one(
            {"_id": services.ObjectId(comment_id)}
        )
